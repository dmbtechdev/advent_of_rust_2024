...
impl Kid {
    ...
    pub fn parse_row(csv_row: &str) -> Result<Kid, ParseError> {
        ...
        let name = fields.next().filter(|name| !name.is_empty()).ok_or(ParseError::NoName)?.to_owned();
        // if name.is_empty() {Err(ParseError::NoName)?}
        let good_deeds = fields
            .next()
            .filter(|good_deeds| !good_deeds.is_empty())
            .ok_or(ParseError::NoGoodDeeds)?
            .parse::<u32>()
            .map_err(|_| ParseError::InvalidGoodDeeds)?;
        let bad_deeds = fields
            .next()
            .filter(|bad_deeds| !bad_deeds.is_empty())
