...
impl Kid {
    pub fn parse_row(csv_row: &str) -> Result<Kid, &'static str> {
        ...
        // 🎁 Your code here! 🎁
        let parts = csv_row.split(',').collect::<Vec<&str>>();
        if parts.len() != 3 {
            return Err("Invalid CSV row");
        }

        let name = parts[0].to_string();
        let good_deeds = parts[1].parse().map_err(|_| "Invalid good deeds")?;
        let bad_deeds = parts[2].parse().map_err(|_| "Invalid bad deeds")?;
